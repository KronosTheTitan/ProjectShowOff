...
using Packages.Hinput.Scripts.Pressable;
using Packages.Hinput.Scripts.Utils;
using UnityEngine;

namespace Packages.Hinput.Debugger.Scripts {
    ...
    public class Debugger : MonoBehaviour {
		...
		private void GetNewCurrentButton () {
			...
			if (gamepadMode == GM.anyGamepad) gamepadsToTest.Add(Hinput.Scripts.Utils.Hinput.anyGamepad);
			if (gamepadMode == GM.individualGamepads) gamepadsToTest = Hinput.Scripts.Utils.Hinput.gamepad;
		...
		private void GetNewCurrentStick () {
			if (gamepadMode == GM.individualGamepads) Hinput.Scripts.Utils.Hinput.gamepad.ForEach(UpdateCurrentStickFromGamepad);
			else UpdateCurrentStickFromGamepad(Hinput.Scripts.Utils.Hinput.anyGamepad);
		}
		...
		private void TestVibration () {
			if (gamepadMode == GM.anyGamepad) TestVibrationOnGamepad(Hinput.Scripts.Utils.Hinput.anyGamepad);
			if (gamepadMode == GM.individualGamepads) TestVibrationOnGamepad(Hinput.Scripts.Utils.Hinput.gamepad[gamepadToVibrate]);
		}
