...
namespace Packages.Hinput.Scripts.Gamepad {
	...
	public class Vibration {
	    ...
	    public Vibration (int index) {
		    if (Utils.Utils.os != Utils.Utils.OS.Windows) return;
		    if (index > 3) return;
		...
		public void Vibrate (float left, float right, float duration) {
			if (canVibrate) Utils.Utils.Coroutine(_Vibrate(left, right, duration));
			else Utils.Utils.VibrationNotAvailableError();
		}

		public void Vibrate(AnimationCurve leftCurve, AnimationCurve rightCurve) {
			if (canVibrate) Utils.Utils.Coroutine(_Vibrate(leftCurve, rightCurve));
			else Utils.Utils.VibrationNotAvailableError();
		}
		...
		public void Vibrate(VibrationPreset vibrationPreset, float left, float right, float duration) {
			if (canVibrate) {
				...
				Utils.Utils.Coroutine(_Vibrate(presetCurves.leftCurve, presetCurves.rightCurve));
			}
			else Utils.Utils.VibrationNotAvailableError();
		}
		...
		public void StopVibration(float duration) {
			if (canVibrate) {
				Utils.Utils.StopRoutines();
				Utils.Utils.Coroutine(_StopVibration(duration));
			}
		}
