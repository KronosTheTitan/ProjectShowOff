...
namespace Packages.Hinput.Scripts.Pressable {
    ...
    public class Trigger : GamepadPressable {
    	...
    	public Trigger (string pressableName, Gamepad.Gamepad gamepad, int index, bool isEnabled) : 
        ...
        private float measuredPosition { 
	        get { // Triggers range from -1 to 1 on Mac, and from 0 to 1 on Windows and Linux.
		        if (Utils.Utils.os == Utils.Utils.OS.Mac) return (Utils.Utils.GetAxis(Utils.Utils.os + "_" + name) + 1)/2;
		        else return Utils.Utils.GetAxis(Utils.Utils.os + "_" + name);	
	        }
        }
